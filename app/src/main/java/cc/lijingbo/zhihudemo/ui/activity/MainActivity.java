...
import cc.lijingbo.zhihudemo.utils.NetWorkUtils;
...
public class MainActivity extends AppCompatActivity
    ...
    private void initView() {
        ...
        mAdapter.setOnItemClickListener(new ZhiHLatestAdapter.OnItemClickListener() {
            @Override
            public void itemClick(View v, int id, int position) {
    ...
    @Override
    public void updateAppBarTitle(String name) {
    ...
    @Override
    public void updateListData(List<ZhiHNewsBean.StoryBean> stories,
    ...
    @Override
    public void updateThemesData(List<ThemesBean.Theme> list) {
        mThemesList.clear();
    ...
    @Override
    public void loadMore(List<ZhiHNewsBean.StoryBean> stories) {
        storiesList.addAll(stories);
        mAdapter.notifyDataSetChanged();
    }
    ...
    @Override
    public void stopLoadMoreStates() {
        isLoading = false;
    }

    @Override
    public void startLoadMoreStates() {
        isLoading = true;
    }

    @Override
    public void onRefresh() {
        if (NetWorkUtils.getNetWordType(MainActivity.this)!=-1){
            if (!isRefresh) {
            ...
            Snackbar.make(mRecyclerView, "没有网络，加载本地数据", Snackbar.LENGTH_SHORT).show();
            mSwipeRefreshLayout.setRefreshing(false);
            isRefresh = false;
            iZhiHPresenter.getZhiHFromCache();
        }
